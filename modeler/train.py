...
def sample(datafile, vocab_map, vocab_freq, author_map):
    with open(datafile, 'r') as file_handler:
        ...
        for name, author in reader:
            ...
            # weights = [1.0 - vocab_freq[i] for i in label]
            # weights = [1.0 for i in label]
            weights = [(i+1.0)/len(label) for i in range(len(label))]
            yield seq, label, weights, [author_map[author]] * len(seq)
...
def train(savedir, params):
    ...
    with open('{}.meta'.format(params.datafile), 'rb') as handler:
        ...
        vocab_map, vocab_freq, author_map = pickle.load(handler)
        vocab_size = len(vocab_map)
        # vocab_freq = np.array(vocab_freq) / max(vocab_freq)
        author_size = len(author_map)

    dataset = tf.data.Dataset.from_generator(
        lambda: sample(params.datafile, vocab_map, vocab_freq, author_map),
        (tf.int32, tf.int32, tf.float32, tf.int32),
    ). \
      shuffle(buffer_size=10000). \
      repeat(params.num_epochs). \
      padded_batch(params.batch_size, ([None,], [None,], [None,], [None,]))
    
    iterator = dataset.make_one_shot_iterator()
    sequences, labels, weights, authors = iterator.get_next()
